...
function getQuotes($conn, $params = []) {
    ...
    if (isset($params['author_id'])) {
        $where[] = "quotes.author_id = $" . (count($paramValues) + 1);
        $paramValues[] = $params['author_id'];
    }
    if (isset($params['category_id'])) {
        $where[] = "quotes.category_id = $" . (count($paramValues) + 1);
...
function createQuote($conn, $data) {
    ...
    if ($result && pg_num_rows($result) > 0) {
        ...
        $result = ['id' => $row['id'], 'quote' => $data['quote'], 'author_id' => $data['author_id'], 'category_id' => $data['category_id']];
        pg_free_result($result);
        return $result;
    } else {
...
function createAuthor($conn, $data) {
    ...
    if ($result && pg_num_rows($result) > 0) {
        $row = pg_fetch_assoc($result);
        $result = ['id' => $row['id'], 'author' => $data['author']];
        pg_free_result($result);
        return $result;
    } else {
        http_response_code(500);
...
function createCategory($conn, $data) {
    ...
    if ($result && pg_num_rows($result) > 0) {
        $row = pg_fetch_assoc($result);
        $result = ['id' => $row['id'], 'category' => $data['category']];
        pg_free_result($result);
        return $result;
    } else {
        http_response_code(500);
...
function updateQuote($conn, $data) {
    ...
    if (!authorExists($conn, <span class="math-inline">data\['author\_id'\]\)\) \{
http\_response\_code\(404\); // Not Found
return \['message' \=\> 'author\_id Not Found'\];
\}
if \(\!categoryExists\(</span>
